...
// c++ -std=c++98 tests/config_parser/confParsertest.cpp src/config/config.cpp src/logging/logger.cpp src/config/directivesParsers.cpp src/helpers/helpers.cpp -o confParsertest
...
int main(int argc, char *argv[])
{
    ...
    for (size_t s = 0; s < servers.size(); ++s)
    {
        ...
        for (size_t i = 0; i < data.locations.size(); i++)
        {
            ...
            for (std::map<int, std::string>::const_iterator it = loc.error_pages.begin(); it != loc.error_pages.end();
                 ++it)
