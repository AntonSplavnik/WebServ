...
bool waitForResponseWithPoll(int socketFd, int timoutMilliseconds)
{
...
void sendData(const std::vector<HTTPTest> &tests, sockaddr_in serverAddress)
{
	for (size_t i = 0; i < tests.size(); ++i)
	{
		...
		if (waitForResponseWithPoll(clientSocket, POLL_TIMOUT * 15))
		{
			...
			{
				buffer[bytesRead < BUFFER_SIZE ? bytesRead : BUFFER_SIZE - 1] = '\0';
