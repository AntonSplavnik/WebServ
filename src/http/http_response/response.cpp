...
void HttpResponse::generateResponse(int statusCode) {
	...
	if (_statusCode >= 300 && _statusCode < 400) {
		// Provide a small HTML body so browsers render instead of prompting a download.
		// Also include the redirect target as a link when available.
		std::string loc = _redirectLocation;
		std::ostringstream b;
		b << "<!DOCTYPE html>\n<html><head><meta charset=\"utf-8\"><title>" << _statusCode << " " << _reasonPhrase << "</title></head>\n<body>\n";
		b << "<h1>" << _reasonPhrase << "</h1>\n";
		if (!loc.empty()) {
			b << "<p>Resource moved: <a href=\"" << loc << "\">" << loc << "</a></p>\n";
		}
		b << "</body></html>\n";
		_body = b.str();
...
void HttpResponse::buildHttpResponse() {
	...
	// Add Location header for redirects (3xx status codes)
	if (_statusCode >= 300 && _statusCode < 400 && !_redirectLocation.empty()) {
		oss << "Location: " << _redirectLocation << "\r\n";
	}
...
void HttpResponse::setRedirectLocation(const std::string& location) {_redirectLocation = location;}
